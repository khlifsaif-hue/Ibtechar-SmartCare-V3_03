...
export async function updateAsset(id: string, input: Row) {
  ...
  const row: Row = { name: next.name, manufacturer: next.manufacturer, model: next.model, serial_number: next.serialNumber, category: next.category, location: next.location, quantity: Math.max(1,num(next.quantity,1)), notes: next.notes || "", source_link: next.sourceLink || "", warranty_until: next.warrantyUntil || null, status: normalizedStatus, tone: statusTone, health: next.health, owner: next.owner, project_id: await projectIdByName(next.project) };
