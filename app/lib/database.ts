...
export type PurchaseOrderRecord = { id: string; date: string; supplier: string; equipment: string; equipmentType: "Asset" | "Consumable"; budgetBranch: string; project: string; location: string; quantity: number; unitPrice: number; total: number; status: string; assetCreated: boolean };
...
export async function listAssets() {
  const supabase = await createClient(); const { data, error } = await supabase.from("assets").select("*,projects(name),asset_financials(*),alerts(count)").order("name"); fail(error);
...
export async function findAsset(id: string) {
  const supabase = await createClient(); const { data, error } = await supabase.from("assets").select("*,projects(name),asset_financials(*),alerts(count)").eq("id", id).maybeSingle(); fail(error); if (!data) return undefined;
...
function assetInsert(input: Partial<AssetRecord> & Pick<AssetRecord, "name" | "category">, id = `AST-${Date.now().toString().slice(-8)}`) {
  ...
  return { id, name: input.name, manufacturer: input.manufacturer || "To be confirmed", model: input.model || "To be confirmed", serial_number: input.serialNumber || "Pending", category: input.category, location: input.location || "IBTECHAR_STORE", installed_on: input.installedOn || today, warranty_until: input.warrantyUntil || null, status: input.status || "Setup required", tone: input.tone || "due", health: input.health ?? 70, uptime: input.uptime ?? 0, metric_label: input.metricLabel || "Monitoring", metric: input.metric || "Pending", maintenance_label: input.maintenanceLabel || "Baseline inspection", maintenance: input.maintenance || "Required", data_source: input.dataSource || "Manual entry", connectivity: input.connectivity || "Not connected", last_seen: null, owner: input.owner || "Technical Services", image_path: undefined };
}
...
export async function updateAsset(id: string, input: Row) {
  ...
  const row: Row = { name: next.name, manufacturer: next.manufacturer, model: next.model, serial_number: next.serialNumber, category: next.category, location: next.location, warranty_until: next.warrantyUntil || null, status: next.status, tone: next.tone, health: next.health, owner: next.owner, project_id: await projectIdByName(next.project) };
...
const importAliases: Record<string,string> = { assetid:"asset_id",asset_id:"asset_id",assetname:"asset_name",asset_name:"asset_name",serialnumber:"serial_number",serial_number:"serial_number",purchasedate:"purchase_date",purchase_date:"purchase_date",acquisitioncostqar:"acquisition_cost_qar",acquisition_cost_qar:"acquisition_cost_qar",residualvalueqar:"residual_value_qar",residual_value_qar:"residual_value_qar",usefullifeyears:"useful_life_years",useful_life_years:"useful_life_years",warrantyuntil:"warranty_until",warranty_until:"warranty_until" };
...
function poFromRow(row:Row):PurchaseOrderRecord{return{id:text(row.id),date:text(row.po_date),supplier:text(row.supplier),equipment:text(row.equipment),equipmentType:text(row.equipment_type)==="Consumable"?"Consumable":"Asset",budgetBranch:text(row.budget_branch),project:relName(row.projects),location:text(row.location),quantity:num(row.quantity,1),unitPrice:num(row.unit_price),total:num(row.total),status:text(row.status),assetCreated:Boolean(row.asset_created)};}
export async function listPurchaseOrders(){const supabase=await createClient();const{data,error}=await supabase.from("purchase_orders").select("*,projects(name)").order("po_date",{ascending:false});fail(error);return(data||[]).map(row=>poFromRow(row as Row));}
export async function createPurchaseOrder(input:Row){const supabase=await createClient();const{data:{user}}=await supabase.auth.getUser();const equipmentType=text(input.equipmentType)==="Consumable"?"Consumable":"Asset";const quantity=Math.max(1,num(input.quantity,1)),unitPrice=num(input.unitPrice);const id=text(input.id)||`IBT-PO-${Date.now().toString().slice(-7)}`,projectId=await projectIdByName(text(input.project));const{data,error}=await supabase.from("purchase_orders").insert({id,project_id:projectId,po_date:text(input.date)||new Date().toISOString().slice(0,10),supplier:text(input.supplier),equipment:text(input.equipment),equipment_type:equipmentType,budget_branch:text(input.budgetBranch,equipmentType==="Asset"?"Equipment":"Consumables"),location:text(input.location),quantity,unit_price:unitPrice,status:text(input.status,"Review"),created_by:user?.id||null}).select("*,projects(name)").single();fail(error);await recalculateProjectFinancials(projectId);return poFromRow(data as Row);}
async function createAssetsFromApprovedPo(order:PurchaseOrderRecord){if(order.equipmentType!=="Asset"||order.assetCreated)return;const supabase=await createClient(),projectId=await projectIdByName(order.project);const inputs=Array.from({length:order.quantity},(_,index)=>{const id=`AST-${order.id.replace(/[^A-Z0-9]/gi,"").slice(-6)}-${String(index+1).padStart(2,"0")}`,input={name:order.quantity>1?`${order.equipment} #${index+1}`:order.equipment,category:"Procured Asset",project:order.project,location:order.location,acquisitionCost:order.unitPrice,residualValue:0,usefulLifeYears:5,purchaseDate:order.date};return{id,input,row:{...assetInsert(input,id),project_id:projectId,status:"Awaiting commissioning"}}});fail((await supabase.from("assets").upsert(inputs.map(x=>x.row),{onConflict:"id",ignoreDuplicates:true})).error);fail((await supabase.from("asset_financials").upsert(inputs.map(x=>assetFinance(x.input,x.id,projectId)),{onConflict:"asset_id"})).error);fail((await supabase.from("purchase_orders").update({asset_created:true}).eq("id",order.id)).error);}
export async function updatePurchaseOrder(id:string,input:Row){const current=(await listPurchaseOrders()).find(order=>order.id===id);if(!current)return undefined;const next={...current,...input};const supabase=await createClient();const quantity=Math.max(1,num(next.quantity,1)),unitPrice=num(next.unitPrice);const{data,error}=await supabase.from("purchase_orders").update({po_date:next.date,supplier:next.supplier,equipment:next.equipment,equipment_type:next.equipmentType,budget_branch:next.budgetBranch,project_id:await projectIdByName(next.project),location:next.location,quantity,unit_price:unitPrice,status:next.status}).eq("id",id).select("*,projects(name)").single();fail(error);const order=poFromRow(data as Row);if(["received","received damaged","delivered"].includes(order.status.toLowerCase()))await createAssetsFromApprovedPo(order);await recalculateProjectFinancials(data.project_id);return order;}
...
function workOrderFromRow(row:Row){return{id:text(row.id),assetId:text(row.asset_id),maintenanceTaskId:row.maintenance_task_id?text(row.maintenance_task_id):null,title:text(row.title),description:text(row.description),status:text(row.status),priority:text(row.priority),assignee:text(row.assignee),dueDate:text(row.due_date),scheduledStart:text(row.scheduled_start),startedAt:row.started_at?text(row.started_at):null,cancelledAt:row.cancelled_at?text(row.cancelled_at):null,createdAt:text(row.created_at),updatedAt:text(row.updated_at),assetName:relName(row.assets),assetModel:Array.isArray(row.assets)?text((row.assets[0] as Row)?.model):text((row.assets as Row|null)?.model)};}
...
export async function assetBundle(id:string){const supabase=await createClient();const[asset,tasks,orders,events,alerts]=await Promise.all([findAsset(id),supabase.from("maintenance_tasks").select("*,work_orders(id)").eq("asset_id",id),supabase.from("work_orders").select("*,assets(name,model)").eq("asset_id",id),supabase.from("service_events").select("*").eq("asset_id",id),supabase.from("alerts").select("*").eq("asset_id",id).eq("resolved",false)]);fail(tasks.error);fail(orders.error);fail(events.error);fail(alerts.error);return{asset,tasks:(tasks.data||[]).map(row=>({id:row.id,assetId:row.asset_id,title:row.title,cadence:row.cadence,nextDue:row.next_due,status:row.status,source:row.source,workOrderId:Array.isArray(row.work_orders)?row.work_orders[0]?.id:null})),orders:(orders.data||[]).map(row=>workOrderFromRow(row as Row)),events:(events.data||[]).map(row=>({id:row.id,assetId:row.asset_id,type:row.type,title:row.title,performedBy:row.performed_by,performedAt:row.performed_at,notes:row.notes})),alerts:(alerts.data||[]).map(row=>({id:row.id,assetId:row.asset_id,severity:row.severity,title:row.title,message:row.message,triggeredAt:row.triggered_at}))};}

async function recalculateProjectFinancials(projectId:string){const supabase=await createClient();const[{data:expenses,error:expenseError},{data:requests,error:requestError},{data:orders,error:orderError}]=await Promise.all([supabase.from("project_expenses").select("amount").eq("project_id",projectId),supabase.from("purchase_requests").select("amount,status").eq("funding_project_id",projectId),supabase.from("purchase_orders").select("total,status").eq("project_id",projectId)]);fail(expenseError);fail(requestError);fail(orderError);const spent=(expenses||[]).reduce((sum,row)=>sum+num(row.amount),0)+(orders||[]).filter(row=>["received","received damaged","delivered"].includes(text(row.status).toLowerCase())).reduce((sum,row)=>sum+num(row.total),0);const committed=(requests||[]).filter(row=>!["rejected","cancelled"].some(status=>text(row.status).toLowerCase().includes(status))).reduce((sum,row)=>sum+num(row.amount),0)+(orders||[]).filter(row=>!["rejected","received","received damaged","delivered"].includes(text(row.status).toLowerCase())).reduce((sum,row)=>sum+num(row.total),0);const{data:existing,error:readError}=await supabase.from("project_financials").select("approved_budget").eq("project_id",projectId).maybeSingle();fail(readError);if(existing)fail((await supabase.from("project_financials").update({spent,committed}).eq("project_id",projectId)).error);}
