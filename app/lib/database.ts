...
export type PurchaseOrderRecord = { id: string; date: string; supplier: string; equipment: string; equipmentType: "Asset" | "Consumable"; budgetBranch: string; project: string; location: string; quantity: number; unitPrice: number; total: number; status: string; assetCreated: boolean; purchaseRequestId?: string; approvedBy?: string; approvedAt?: string };
...
function assetFromRow(row: Row): AssetRecord {
  ...
  return {
    ...
    quantity: Math.max(1, num(row.quantity, 1)), notes: text(row.notes), sourceLink: text(row.source_link),
  };
}
...
export async function updateAsset(id: string, input: Row) {
  ...
  const row: Row = { name: next.name, manufacturer: next.manufacturer, model: next.model, serial_number: next.serialNumber, category: next.category, location: next.location, quantity: Math.max(1,num(next.quantity,1)), notes: next.notes || "", source_link: next.sourceLink || "", warranty_until: next.warrantyUntil || null, status: next.status, tone: next.tone, health: next.health, owner: next.owner, project_id: await projectIdByName(next.project) };
...
export async function updatePurchaseOrder(id:string,input:Row,approvedBy?:string){const current=(await listPurchaseOrders()).find(order=>order.id===id);if(!current)return undefined;const next={...current,...input};const supabase=await createClient();const quantity=Math.max(1,num(next.quantity,1)),unitPrice=num(next.unitPrice);const status=["Review","Approved","Rejected"].includes(text(next.status))?text(next.status):current.status;const approvalFields=status==="Approved"?{approved_by:approvedBy||null,approved_at:new Date().toISOString()}:{approved_by:null,approved_at:null};const{data,error}=await supabase.from("purchase_orders").update({po_date:next.date,supplier:next.supplier,equipment:next.equipment,equipment_type:next.equipmentType,budget_branch:next.budgetBranch,project_id:await projectIdByName(next.project),location:next.location,quantity,unit_price:unitPrice,status,...approvalFields}).eq("id",id).select("*,projects(name),approved_by_profile:profiles!purchase_orders_approved_by_fkey(full_name)").single();fail(error);const order=poFromRow(data as Row);if(order.status==="Approved")await createAssetsFromApprovedPo(order);if(data.purchase_request_id)await supabase.from("purchase_requests").update({status:status==="Approved"?"PO approved":status==="Rejected"?"PO rejected":"PO in review"}).eq("id",data.purchase_request_id);await notifyProject(data.project_id,"purchase_order",id,`Purchase order ${status.toLowerCase()}`,`${id} is ${status.toLowerCase()}.`);await recalculateProjectFinancials(data.project_id);return order;}
